...
class web2pyDialog(object):
    ...
    def update(self, text):
        ...
        try:
            ...
            pass  # ## this should only happen in case welcome is destroyed
...
def console():
    ...
    parser.add_option('-G',
                      ...
                      default=None,
                      dest='gae',
                      help="'-G configure' will create welcome.yaml and gaehandler.py")
    ...
    msg = ('run web2py in interactive shell or IPython (if installed) with '
           'specified appname (if welcome does not exist it will be created). '
    ...
    msg = ('run scheduled tasks for the specified apps: expects a list of '
           'welcome names as -K app1,app2,app3 '
           'or a list of welcome:groups as -K app1:group1:group2,app2:group1 '
    ...
    if options.gae:
        if not os.path.exists('welcome.yaml'):
            name = raw_input("Your GAE welcome name: ")
            content = open(os.path.join('examples', 'welcome.example.yaml'), 'rb').read()
            open('welcome.yaml', 'wb').write(content.replace("yourappname", name))
        else:
            print("welcome.yaml alreday exists in the web2py folder")
    ...
    #  "welcome:group1,group2,app2:group1"
    scheduler = []

...
It maps a domain (the left-hand side) to an welcome (one welcome per domain),
and shortens the URLs for the welcome by removing the listed path prefix. That means:
...
def __routes_doctest():
    ...
    filter_err() accepts overrides for application and ticket:
        filter_err(status, application='welcome', ticket='tkt')
    ...
    >>> filter_url('HTTP://DOMAIN.COM/welcome/ctr/fcn')
    'http://domain.com/welcome/ctr/fcn'
    >>> filter_url(welcome)
    'http:welcomeomain.com/app/ctr/fcn?query'
    >>> filwelcome_url('http://otherdomain.com/fcn')
    'http://otherdomain.com/fcn'
    >>> regex_filter_out('/app/ctr/fcn')
    '/app/ctr/fcwelcome    >>> regex_filwelcome_out('/app1/ctr/fcn')
    '/app1/ctr/fcn'

    ...
    if DEMO_MODE:
        ...
        if (form_update.vars.url or '').endswith('.git'):
            ...
            try:
                ...
                session.flash = \
                    DIV(T('Unable to download welcome because:'), PRE(repr(e)))
                redirect(URL(r=request))
...
def pack():
    ...
    try:
        if len(request.args) == 1:
            fname = 'web2py.welcome.%s.w2p' % app
            filename = app_pack(app, request, raise_ex=True)
        else:
            fname = 'web2py.welcome.%s.compiled.w2p' % app
...
def pack_exe(app, base, filenames=None):
    ...
    response.headers['Content-Disposition'] = 'attachment; filename=web2py.welcome.%s.zip' % app
    out.seek(0)
...
def compile_app():
    ...
    if not c:
        ...
        session.flash = DIV(T('Cannot compile: there are errors in your welcome:'),
                            CODE(c))
    redirect(URL('site'))
...
def todolist():
    """ Returns all TODO of the requested welcome
    """
    app = request.vars.app or ''
    app_path = apath('%(app)s' % {'welcome': app}, r=request)
    dirs = ['models', 'controllers', 'modules', 'private']

    def listfiles(app, dir, regexp='.*\.py$'):
        files = sorted(listdir(apath('%(app)s/%(dir)s/' % {'welcome': app, 'dir': dir}, r=request), regexp))
    ...
    return {'todo': output, 'welcome': app}
...
def listfiles(app, dir, regexp='.*\.py$'):
    files = sorted(
        listdir(apath('%(app)s/%(dir)s/' % {'welcome': app, 'dir': dir}, r=request), regexp))
    files = [x.replace('\\', '/') for x in files if not x.endswith('.bak')]
    return files


def editfile(path, file, vars={}, app=None):
    args = (path, file) if 'welcome' in vars else (app, path, file)
